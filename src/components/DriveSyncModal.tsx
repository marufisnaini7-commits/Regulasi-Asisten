...
interface DriveSyncModalProps {
  ...
  allRegulations?: RegulationDocument[];
...
export const DriveSyncModal: React.FC<DriveSyncModalProps> = ({
  ...
  const isStaticHosting = typeof window !== 'undefined' && (window.location.hostname.includes('github.io') || window.location.hostname.includes('pages.dev'));
  const [folderUrlInput, setFolderUrlInput] = useState(syncConfig.targetFolderUrl || '');
  const [activeTab, setActiveTab] = useState<'link' | 'subfolders' | 'json_backup' | 'manual'>(
    isStaticHosting ? 'json_backup' : 'link'
  );
  ...
  const handleExportJson = () => {
    try {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(allRegulations, null, 2));
      const downloadAnchor = document.createElement('a');
      downloadAnchor.setAttribute("href", dataStr);
      downloadAnchor.setAttribute("download", `database_regulasi_karantina_asn_${new Date().toISOString().slice(0, 10)}.json`);
      document.body.appendChild(downloadAnchor);
      downloadAnchor.click();
      downloadAnchor.remove();
      setSuccessMessage(`Berhasil mengekspor ${allRegulations.length} dokumen regulasi ke berkas .json!`);
      setError(null);
    } catch (err: any) {
      setError(`Gagal mengekspor berkas: ${err.message}`);
    }
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          onBulkUpdateRegulations(parsed);
          setSuccessMessage(`Berhasil mengimpor ${parsed.length} dokumen regulasi dari berkas JSON ke penyimpanan lokal!`);
          setError(null);
        } else {
          setError('Format berkas JSON tidak sesuai. Berkas harus berisi array dokumen regulasi.');
        }
      } catch (err: any) {
        setError(`Gagal membaca berkas JSON: ${err.message}`);
      }
    };
  ...
  // 3. Batch text files import (.txt, .md)
  const handleBatchTextFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    const loadedDocs: RegulationDocument[] = [];
    let processed = 0;
      ...
      reader.onload = (ev) => {
        ...
        if (processed === total) {
          if (loadedDocs.length > 0) {
            onBulkUpdateRegulations(loadedDocs);
            setSuccessMessage(`Berhasil mengimpor ${loadedDocs.length} dokumen dari berkas teks ke database!`);
            setError(null);
          } else {
            setError('Tidak ada isi teks yang terbaca dari berkas yang dipilih.');
          }
        }
      };
      reader.readAsText(file);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl overflow-hidden my-6 transition-all">
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          {/* Static Hosting Notification (GitHub Pages) */}
          {isStaticHosting && (
            <div className="p-4 bg-amber-50/90 border border-amber-200/90 rounded-xl text-xs text-amber-900 space-y-2">
              <div className="flex items-center gap-2 font-bold text-amber-950">
                <Info className="w-4 h-4 text-amber-700 shrink-0" />
                <span>Pemberitahuan Khusus GitHub Pages (Hosting Statis):</span>
              </div>
              <p className="text-[11.5px] leading-relaxed text-amber-800">
                GitHub Pages adalah layanan hosting statis dan tidak menjalankan server backend Node.js. Panggilan scraper link Google Drive dibatasi oleh browser karena kebijakan keamanan CORS Google.
              </p>
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setActiveTab('json_backup')}
                  className="bg-amber-700 hover:bg-amber-800 text-white px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition"
                >
                  <Database className="w-3.5 h-3.5" />
                  <span>Gunakan Tab Impor / Ekspor JSON (Instan)</span>
                </button>
                <span className="text-[11px] text-amber-700">
                  atau deploy repositori ke <strong>Render.com</strong> untuk backend otomatis.
                </span>
              </div>
            </div>
          )}
          ...
          {/* TAB: Impor / Ekspor Database JSON (Solusi Cepat untuk GitHub Pages & Backup) */}
          {activeTab === 'json_backup' && (
            <div className="space-y-5">
              <div className="p-4 bg-indigo-50/70 border border-indigo-200/80 rounded-xl text-xs text-indigo-950 space-y-1.5">
                <div className="flex items-center gap-2 font-bold text-indigo-900">
                  <Database className="w-4 h-4 text-indigo-600" />
                  <span>Cadangkan & Pindahkan Seluruh Database Regulasi</span>
                </div>
                <p className="text-[11.5px] leading-relaxed text-indigo-800">
                  Fitur ini memungkinkan Anda menyimpan seluruh regulasi (beserta pasal & diktum) ke dalam 1 file <code>.json</code>, atau memasukkan file cadangan ke browser perangkat lain tanpa memerlukan koneksi backend.
                </p>
              </div>

              {/* Action 1: Export Current Database */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4.5 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h4 className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                      <Download className="w-4 h-4 text-indigo-600" />
                      Ekspor Database Saat Ini ({allRegulations.length} Regulasi)
                    </h4>
                    <p className="text-[11.5px] text-slate-500 mt-1">
                      Unduh seluruh berkas regulasi yang saat ini tersimpan di aplikasi ini ke komputer Anda dalam format .JSON.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={handleExportJson}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition flex items-center gap-1.5 shrink-0 shadow-2xs"
                  >
              ...
              {/* Action 2: Import JSON Database */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4.5 space-y-3">
                <div className="space-y-1">
                  <h4 className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                    <Upload className="w-4 h-4 text-emerald-600" />
                    Impor Database Regulasi (.JSON)
                  </h4>
                  <p className="text-[11.5px] text-slate-500">
                    Pilih file cadangan .json yang sebelumnya Anda unduh. Seluruh regulasi akan langsung tersimpan di memori browser ini.
                  </p>
                </div>

                <label className="border-2 border-dashed border-slate-300 hover:border-indigo-400 rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer bg-white transition group">
                  ...
                  <div className="text-center">
                    <span className="text-xs font-bold text-indigo-600 group-hover:underline">
                      Klik untuk memilih berkas .json
                    </span>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      Format: database_regulasi_karantina_asn.json
                    </p>
                  </div>
                  <input
                    type="file"
              ...
              {/* Action 3: Batch Text / Markdown files */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4.5 space-y-3">
                <div className="space-y-1">
                  <h4 className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                    <FileUp className="w-4 h-4 text-blue-600" />
                    Unggah Berkas Teks Sekaligus (.TXT / .MD)
                  </h4>
                  <p className="text-[11.5px] text-slate-500">
                    Pilih beberapa file teks regulasi dari komputer Anda secara bersamaan. Sistem akan mengekstrak Bab & Pasal otomatis.
                  </p>
                </div>

                <label className="border border-slate-300 hover:border-indigo-400 rounded-xl p-3 flex items-center justify-between gap-3 cursor-pointer bg-white transition group">
                  <div className="flex items-center gap-2 text-xs text-slate-600">
                    <FileUp className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 transition" />
                    <span className="font-medium group-hover:text-indigo-600">
                      Pilih satu atau beberapa file .txt / .md
                    </span>
                  </div>
                  <span className="bg-slate-100 group-hover:bg-indigo-50 text-slate-700 group-hover:text-indigo-700 px-2.5 py-1 rounded text-xs font-semibold">
                    Pilih Berkas
                  </span>
                  <input
                    type="file"
